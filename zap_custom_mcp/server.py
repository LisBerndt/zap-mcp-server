...
class ScanManager:
    ...
    def start_scan(
        self, scan_id: str, scan_type: str, target: str, args: Dict[str, Any]
    ) -> str:
        ...
        LOG.info(
            f"Started {scan_type} scan",
            extra={"extra": {"scan_id": scan_id, "target": target}},
        )
        return scan_id
    ...
    def _run_scan(
        ...
        try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        clear_response = requests.get(
                            clear_url, params=clear_params, timeout=10
                        )
                        if clear_response.status_code == 200:
                            LOG.info(f"Cleared all alerts for scan {scan_id}")
                        else:
                            LOG.warning(
                                f"Failed to clear alerts for scan {scan_id}: HTTP {clear_response.status_code}"
                            )
                    except Exception as e:
                        LOG.warning(
                            f"Exception clearing alerts for scan {scan_id}: {e}"
                        )

                else:
                    LOG.warning(
                        f"Failed to create new session for scan {scan_id}, using existing session"
                    )
            except Exception as e:
                LOG.warning(
                    f"Exception creating new session for scan {scan_id}: {e}, using existing session"
                )

            # Create progress callback
            def progress_callback(
                scan_id: str, scan_phase: str, progress: int, message: str
            ):
                with self.lock:
            ...
            if scan_type == "active":
                ...
                result = self._run_complete_scan(
                    scan_id, target, args, progress_callback
                )
    ...
    def _run_active_scan(
        ...
        try:
            ...
            LOG.warning(
                f"Spider failed, continuing with active scan",
                extra={"extra": {"scan_id": scan_id, "error": str(e)}},
            )
        ...
        result["activeScan"] = {
            "scanId": ascan_id,
            "duration_seconds": round(ascan_duration, 2),
        }
        return result

    def _run_complete_scan(
        self,
        scan_id: str,
        target: str,
        args: Dict[str, Any],
        ...
        if args.get("wait_for_passive", True):
            ...
            pscan_duration = pscan_wait(
                scan_id,
                args.get("passive_poll_interval_seconds", 0.5),
                args.get("passive_timeout_seconds", 600),
                session_id=ZAP_SESSION_ID,
            )
        ...
        result = _finalize(
            ...
            args.get("include_evidence", False),
            {
                "pscan": pscan_duration,
                "ajax": ajax_duration,
                "spider": spider_duration,
                "ascan": ascan_duration,
            },
            mode="complete",
        )
        result.update(
            {
                ...
                "passiveScan": {
                    "waited": bool(args.get("wait_for_passive", True)),
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "scanPolicyName": {
                        "type": "string",
                        "description": "Name of the scan policy to use",
                    },
                    ...
                    "ascan_max_wait_seconds": {
                        "type": "integer",
                        "default": 7200,
                        "description": "Maximum wait time for active scan (default: 2 hours)",
                    },
                    "spider_max_wait_seconds": {
                        "type": "integer",
                        "default": 1800,
                        "description": "Maximum wait time for spider (default: 30 minutes)",
                    },
                    "include_findings": {
                        "type": "boolean",
                        "default": True,
                        "description": "Include findings in results",
                    },
                    "include_evidence": {
                        "type": "boolean",
                        "default": False,
                        "description": "Include evidence in results",
                    },
                },
                "required": ["url"],
            },
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "ajax_maxCrawlDepth": {
                        "type": "integer",
                        "default": 10,
                        "description": "Maximum crawl depth for AJAX spider",
                    },
                    "ajax_maxCrawlStates": {
                        "type": "integer",
                        "default": 0,
                        "description": "Maximum crawl states for AJAX spider",
                    },
                    "ajax_maxDuration": {
                        "type": "integer",
                        "default": 60,
                        "description": "Maximum duration for AJAX spider",
                    },
                    "ajax_eventWait": {
                        "type": "integer",
                        "default": 1000,
                        "description": "Event wait time for AJAX spider",
                    },
                    "ajax_clickDefaultElems": {
                        "type": "boolean",
                        "default": True,
                        "description": "Click default elements in AJAX spider",
                    },
                    "ajax_clickElemsOnce": {
                        "type": "boolean",
                        "default": True,
                        "description": "Click elements once in AJAX spider",
                    },
                    "ajax_browserId": {
                        "type": "string",
                        "description": "Browser ID for AJAX spider",
                    },
                    "ajax_wait_seconds": {
                        "type": "integer",
                        "default": 300,
                        "description": "Wait time for AJAX spider",
                    },
                    "wait_for_passive": {
                        "type": "boolean",
                        "default": True,
                        "description": "Wait for passive scan to complete",
                    },
                    "passive_poll_interval_seconds": {
                        "type": "number",
                        "default": 0.5,
                        "description": "Passive scan polling interval",
                    },
                    "passive_timeout_seconds": {
                        "type": "integer",
                        "default": 600,
                        "description": "Passive scan timeout",
                    },
                    "disable_websocket": {
                        "type": "boolean",
                        "default": False,
                        "description": "Disable websocket scanning",
                    },
                },
                "required": ["url"],
            },
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "timeout_seconds": {
                        "type": "integer",
                        "default": 600,
                        "description": "Maximum wait time for passive scan",
                    },
                },
                "required": ["url"],
            },
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "scan_id": {
                        "type": "string",
                        "description": "Scan ID returned by start_*_scan",
                    }
                },
                "required": ["scan_id"],
            },
...
async def call_tool(name: str, arguments: Dict[str, Any]) -> list[TextContent]:
    if name == "start_active_scan":
        scan_id = scan_manager.start_scan(
            uuid.uuid4().hex[:8], "active", arguments["url"], arguments
        )
        result = {
            "scan_id": scan_id,
            "status": "started",
            "message": "Active scan started in background",
        }
        return [TextContent(type="text", text=str(result))]
    elif name == "start_complete_scan":
        scan_id = scan_manager.start_scan(
            uuid.uuid4().hex[:8], "complete", arguments["url"], arguments
        )
        result = {
            "scan_id": scan_id,
            "status": "started",
            "message": "Complete scan started in background",
        }
        return [TextContent(type="text", text=str(result))]
    elif name == "start_ajax_scan":
        scan_id = scan_manager.start_scan(
            uuid.uuid4().hex[:8], "ajax", arguments["url"], arguments
        )
        result = {
            "scan_id": scan_id,
            "status": "started",
            "message": "AJAX scan started in background",
        }
        return [TextContent(type="text", text=str(result))]
    elif name == "start_passive_scan":
        scan_id = scan_manager.start_scan(
            uuid.uuid4().hex[:8], "passive", arguments["url"], arguments
        )
        result = {
            "scan_id": scan_id,
            "status": "started",
            "message": "Passive scan started in background",
        }
        return [TextContent(type="text", text=str(result))]
        ...
        result = {
            "success": success,
            "message": "Scan cancelled" if success else "Scan not found or not running",
        }
        return [TextContent(type="text", text=str(result))]
