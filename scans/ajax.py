...
class AjaxOptions(BaseModel):
    ...
    browserId: Optional[str] = "firefox-headless"    # Firefox Headless as default, can be explicitly overridden
    wait_seconds: int = 300
    poll_interval_seconds: float = 1.0


def run_ajax(scan_id: str, target: str, a: AjaxOptions, session_id: str) -> Tuple[bool, float, int]:
    ...
    set_opt("/JSON/ajaxSpider/action/setOptionMaxCrawlDepth/", "Integer", a.maxCrawlDepth)
    set_opt("/JSON/ajaxSpider/action/setOptionMaxCrawlStates/", "Integer", a.maxCrawlStates)
    ...
    set_opt("/JSON/ajaxSpider/action/setOptionClickDefaultElems/", "Boolean", a.clickDefaultElems)
    set_opt("/JSON/ajaxSpider/action/setOptionClickElemsOnce/", "Boolean", a.clickElemsOnce)
    set_opt("/JSON/ajaxSpider/action/setOptionNumberOfBrowsers/", "Integer", a.numberOfBrowsers)
    ...
    try:
        ...
        get_json(scan_id, "/JSON/ajaxSpider/action/scan/", params, session_id=session_id)
    except Exception as e:
        LOG.error("ajax.start.error", extra={"extra": {"scan_id": scan_id, "err": repr(e)}})
        return (False, 0.0, 0)
    ...
    while time.monotonic() < deadline:
        try:
            status = get_json(scan_id, "/JSON/ajaxSpider/view/status/", {}, session_id=session_id).get("status", "").lower()
            nr = get_json(scan_id, "/JSON/ajaxSpider/view/numberOfResults/", {}, session_id=session_id).get("numberOfResults")
    ...
    if status not in ("stopped", "complete", "finished"):
        try:
            get_json(scan_id, "/JSON/ajaxSpider/action/stop/", {}, session_id=session_id)
        except Exception:
            pass
