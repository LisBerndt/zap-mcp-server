...
def transform_url_for_docker(url: str) -> str:
    ...
    if parsed.hostname in ('localhost', '127.0.0.1'):
        ...
        new_parsed = parsed._replace(netloc=f"{new_hostname}:{parsed.port}" if parsed.port else new_hostname)
...
def vuln_names_from_findings(findings: List[dict]) -> List[dict]:
    ...
    for finding in findings:
        ...
        counter[(risk, name)] += 1
    items = sorted(counter.items(), key=lambda kv: (_RISK_LEVELS.get(kv[0][0], 9), -kv[1], kv[0][1].lower()))
    return [{"name": name, "risk": risk, "count": count} for (risk, name), count in items]
...
def _finalize(scan_id: str, target: str, include_findings: bool, include_evidence: bool, durations: Dict[str, float], mode: str) -> Dict:
    """Finalize scan results."""
    from alerts import alerts_summary_fast, alerts_all
    ...
    if include_findings:
        ...
        if include_evidence:
            findings = alerts_all(scan_id, target, include_evidence=True, session_id=ZAP_SESSION_ID)
